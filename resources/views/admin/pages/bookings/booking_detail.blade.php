...
<div class="modal-content">
            ...
            <div class="w-100">
              ...
              @if ($bookingDetail->status == 1)
                <div
                  ...
                  role="alert"
                >
                  <strong>Alert:</strong> visitation was approved
                </div>
              @endif
              @if ($bookingDetail->status == 2 || $bookingDetail->status == 3)
                <div
                  ...
                  role="alert"
                >
                  <strong>Alert:</strong> visitation was canceled/declined
                </div>
              @endif
            </div>
          </div>
            ...
            <div class="col-lg-8">
                <table class="table table-sm">
                    ...
                    @elseif($bookingDetail->link_type == "code")
                      <tr>
                        <td colspan="2" class="fw-bold text-center text-white" style="background-color: #098344">
                          <a class="text-white" href="{{ url('/videocall') }}"> Click to join the meeting</a>
                        </td>
                      </tr>
                    @endif
                </table>
    ...
    <div class="modal-footer">
        <button type="button" class="btn btn-dark btn-sm px-4" data-dismiss="modal">Close</button>
        @if ($bookingDetail->status == 0)
        <button type="button" 
          data-bookid="{{ $bookingDetail->bookID }}"
          data-transaction_number="{{ $bookingDetail->transaction_number }}"
          class="btn btn-danger btn-sm px-4 declineRequest">Reject</button>
        <button type="button" 
          data-bookid="{{ $bookingDetail->bookID }}"
          data-transaction_number="{{ $bookingDetail->transaction_number }}"
          class="btn btn-primary btn-sm px-4 approveRequest">Approve</button>
        @endif
    </div>
